...
<div class="mx-auto w-full p-6 md:w-auto md:min-w-[300px]">
	<h1 class="flex w-full items-center justify-between text-4xl font-semibold tracking-tight">
		{#if isOtpStep}
			<button onclick={() => backFromOtp()} class="btn btn-circle btn-ghost">
		...
		<div>
			{#if !isOtpStep}
				<form onsubmit={requestOtp}>
					...
					<!-- > -->
					<input
						class="input-bordered input w-full rounded-xs bg-base-100 input-primary"
						id="email"
						name="email"
						autofocus
					...
					<button
						...
						class="btn mt-6 btn-block rounded-md btn-md btn-primary"
						onclick={requestOtp}
					>
						{#if isLoading}
			...
			{#if isOtpStep}
				<form class="mx-auto flex max-w-lg flex-col justify-center">
					<div class="flex justify-center space-x-2 p-4">
						{#each otp as value, index}
							<input
								...
								maxlength="1"
								pattern="\d*"
								class="h-10 w-10 rounded-sm border bg-base-100 text-center text-lg"
					...
					<button
						...
						class={`btn mt-4 w-full rounded-md btn-primary ${success ? 'bg-green-700' : ''}`}
